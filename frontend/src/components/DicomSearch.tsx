  ...
  return (
    <div className="p-3">
      <div className="flex items-center justify-between mb-2">
        ...
        <button
          onClick={onClose}
          className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 p-1 rounded transition"
          aria-label="Закрыть"
          title="Закрыть"
        >
      ...
      <div className="flex gap-2 mb-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-7 pr-2 py-1 border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 rounded text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
          />
        </div>
        <label className="flex items-center gap-1 text-xs text-slate-700 dark:text-slate-300 whitespace-nowrap">
          <input
            type="checkbox"
      ...
      {error && (
        <div className="bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-xs rounded px-2 py-1 mb-2">
          {error}
        </div>
      )}
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded max-h-48 overflow-y-auto">
        {studies.length === 0 && !loading ? (
          ...
          <table className="w-full text-xs">
            <thead className="bg-slate-50 dark:bg-slate-700/50 sticky top-0">
              <tr>
                <th className="px-2 py-1 text-left text-slate-500 dark:text-slate-400 font-medium">Дата</th>
                <th className="px-2 py-1 text-left text-slate-500 dark:text-slate-400 font-medium">Мод.</th>
                <th className="px-2 py-1 text-left text-slate-500 dark:text-slate-400 font-medium">Пациент</th>
                <th className="px-2 py-1 text-left text-slate-500 dark:text-slate-400 font-medium">Описание</th>
                <th className="px-2 py-1 text-left text-slate-500 dark:text-slate-400 font-medium">Accession</th>
                <th className="px-2 py-1 text-left text-slate-500 dark:text-slate-400 font-medium">RIS</th>
              </tr>
            </thead>
            <tbody>
              {studies.map(s => (
                <tr
                  ...
                  className="hover:bg-brand-50 dark:hover:bg-slate-700/50 cursor-pointer border-t border-slate-100 dark:border-slate-700"
                >
                  <td className="px-2 py-1 text-slate-700 dark:text-slate-300">{s.study_date || '—'}</td>
                  <td className="px-2 py-1">
                    <span className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-700 rounded text-slate-700 dark:text-slate-300">
                      {s.modality || '—'}
                    </span>
                  </td>
                  <td className="px-2 py-1 text-slate-700 dark:text-slate-300 truncate max-w-[160px]">
                    {s.patient_name_dicom || '—'}
                  </td>
                  <td className="px-2 py-1 text-slate-500 dark:text-slate-400 truncate max-w-[200px]">
                    {s.study_description || '—'}
                  </td>
