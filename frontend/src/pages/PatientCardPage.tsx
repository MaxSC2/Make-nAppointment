...
export default function PatientCardPage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <button
        onClick={() => navigate('/patients')}
        className="text-sm text-teal-600 dark:text-teal-400 hover:underline mb-4"
      >
        {t('patientCard.backToPatients')}
      </button>
        ...
        <div className="space-y-2">
          {studies.map(s => (
            <div
              ...
              className="flex items-center gap-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-3"
            >
              <div className="w-20 h-20 flex-shrink-0 bg-slate-100 dark:bg-slate-700 rounded overflow-hidden flex items-center justify-center">
                {imgErrors[s.study_uid] ? (
                  <span className="text-xs text-slate-400 dark:text-slate-500">{t('patientCard.noPreview')}</span>
                ) : (
                  <img
