...
**[ОТ СЕБЯ: укажи свою роль в команде (например, «Backend Lead / Архитектор» или «Frontend разработчик»), перечисли свои основные обязанности, напиши 2-3 предложения о том, с чего начал, что было самым сложным в первые дни практики.]**
...
**[ОТ СЕБЯ: почему ты выбрал именно этот проект / эту тему? Что знал о медицинских ИС до практики? Какие реальные проблемы медучреждений тебя заинтересовали? 2-3 предложения.]**
...
**[ОТ СЕБЯ: как происходил выбор технологий? Обсуждали ли альтернативы (OHIF, DCM4CHEE)? Почему остановились на Orthanc/DWV? Кто принимал решение? 2-3 предложения.]**
...
**[ОТ СЕБЯ: какие технологии из стека ты уже знал до практики, а какие освоил в процессе? Что было самым трудным в изучении? Например: «До практики я знал только Python и SQL, а FastAPI и SQLAlchemy изучал уже по ходу разработки.» 2-3 предложения.]**
...
**[ОТ СЕБЯ: как обсуждали архитектуру? Были ли споры о том, как соединить elqueue с RIS? Почему выбрали REST, а не прямые вызовы? Кто рисовал диаграммы? 2-3 предложения.]**
...
**[ОТ СЕБЯ: какой код писал лично ты? Например: «Я реализовал эндпоинты /api/orders и PACS-фасад», или «Я делал фронтенд — DwvViewer и StudiesPage». Перечисли 2-3 конкретные фичи, которыми гордишься.]**
...
**[ОТ СЕБЯ: как проходила интеграция DWV-просмотрщика? С какими проблемами столкнулся (CORS, передача JWT-токена, загрузка DICOM-файлов)? Кто помогал решать? 2-3 предложения.]**
...
**[ОТ СЕБЯ: как настраивали JWT и роли? Кто отвечал за безопасность? Какие уязвимости нашли при code review? Опиши свой опыт: «Я добавил require_role на create_order и поправил Vite proxy, который обходил JWT.» 2-3 предложения.]**
...
### 5.5. Вторая волна ревью (полный обход кода)

После исправления критических проблем проведён повторный полный обход всей кодовой базы: 611 строк `pacs_facade.py`, все роутеры, модели, зависимости, фронтенд-компоненты, хуки, контекст аутентификации.
...
| # | Проблема | Файл |
|---|----------|------|
| 16 | CORS `allow_origins=["*"]` (dev-режим) | `main.py:62` |
| 17 | 0 тестов (unit / integration / e2e) | весь проект |

**Сводка по двум волнам ревью:**
...
**[ОТ СЕБЯ: чему ты научился за практику? Что было самым ценным опытом? Какие навыки приобрёл (Git, code review, работа с DICOM, FastAPI, безопасность)? Что пошло не так и как исправляли? 3-5 предложений.]**
...
**[ОТ СЕБЯ: напиши, как распределяли задачи внутри команды. Как часто созванивались/списывались? Какие инструменты использовали для коммуникации (Discord, Telegram, GitHub Issues)? Был ли лидер, который раздавал задачи? 2-3 предложения.]**
...
| № | Критерий (из силлабуса) | Баллы | Где в отчёте | [ОТ СЕБЯ: что делал лично ты] |
|---|---|---|---|---|
| 1 | **Понимание IT-сферы и типов IT-проектов** | 10 | Глава 1 (анализ DICOM, PACS-RIS, обзор решений), 6.3 (выводы) | [ОТ СЕБЯ: опиши, как изменилось твоё понимание IT-сферы — например, «До практики я не знал про медицинские ИС, DICOM, HL7. Теперь понимаю, как устроен PACS-сервер, зачем нужен прокси-слой, как врачи работают со снимками.»] |
| 2 | **Знание технологического стека проекта** | 10 | 2.2 (стек), Глава 4 (реализация — FastAPI, PostgreSQL, React, DWV) | [ОТ СЕБЯ: перечисли технологии, которые освоил: «Научился работать с FastAPI, SQLAlchemy 2 async, httpx, pydicom. На фронте — React 19, DWV, Vite proxy.»] |
| 3 | **Владение инструментами (Git, IDE и др.)** | 10 | 5.3 (code review), Приложение 2 (запуск), WHERE_TO_WORK.md, git-история (8+ коммитов) | [ОТ СЕБЯ: опиши работу с git: «Делал commit/push/pull, решал merge conflict, писал commit messages в стиле conventional commits, работал с двумя репозиториями (pacs-ris-queue и ARCHIVE/MedPlatform).»] |
| 4 | **Навыки работы в команде (code review, обсуждения)** | 10 | 5.3-5.5 (две волны code review: 24+17=41 проблема найдена, 10 исправлено), Приложение 3 (роли) | [ОТ СЕБЯ: опиши процесс code review: «Я провёл две волны ревью всего кода: первая — 24 проблемы (3 критические, исправлены сразу), вторая — полный обход, ещё 17 замечаний. Обсуждали каждую находку с командой.»] |
| 5 | **Понимание жизненного цикла и SDLC** | 10 | Главы 1-6 (анализ → ТЗ → проектирование → реализация → тестирование → выводы), Приложение 4 (v1.0→v2.0→v2.1) | [ОТ СЕБЯ: «Проект прошёл 3 итерации: v1.0 (SQLite + без auth) → v2.0 (PostgreSQL + JWT + PACS-фасад) → v2.1 (аудит безопасности + исправление уязвимостей).»] |
| 6 | **Применение методологий Agile/Scrum/Kanban** | 10 | SESSION.md (Daily-логи), Приложение 3 (распределение ролей), git-история (короткие итерации) | [ОТ СЕБЯ: «Мы работали короткими итерациями по 1-2 дня: утром обсуждали задачи, вечером показывали результат. Использовали GitHub для трекинга, SESSION.md как daily-log. Я ставил себе задачи сам и отчитывался перед командой.»] |
| 7 | **Оформление документации и отчётов** | 10 | Весь REPORT.md, ARCHITECTURE.md, CODE_REVIEW.md, SESSION.md, PRACTICE_LOG.md, PROJECT_REPORT.md | [ОТ СЕБЯ: «Я оформил отчёт по ГОСТ-структуре (Введение, 6 глав, Приложения), добавил таблицы, диаграммы, листинги кода. Весь текст — на русском. Приложил code review и аудит безопасности.»] |
| 8 | **Качество командной работы и коммуникации** | 10 | Приложение 3 (роли), SESSION.md (daily-синхронизации), git-история | [ОТ СЕБЯ: опиши взаимодействие: «Созванивались каждый день в Discord, обсуждали блокеры. Я делал ревью чужого кода, писал баг-репорты в SESSION.md. Помогал с настройкой Orthanc и генерацией тестовых DICOM.»] |
| 9 | **Качество продукта (результата разработки)** | 10 | 5.1-5.2 (тестирование: 5 сценариев пройдено), 5.3 (0 ошибок компиляции) | [ОТ СЕБЯ: «Работающий продукт: 9 страниц React-фронтенда, 2 FastAPI-сервиса, PACS-фасад, JWT + RBAC, DICOM-просмотрщик. Все сценарии ручного тестирования пройдены. 10 уязвимостей исправлено.»] |
| 10 | **Защита отчёта (презентация + ответы)** | 10 | Весь REPORT.md + практическая демонстрация системы | [ОТ СЕБЯ: заполни после защиты — как прошла презентация, какие вопросы задавали, что ответил.] |

**[ОТ СЕБЯ ИТОГО: напиши 3-5 предложений — общее впечатление от практики, что понравилось, что нет, какие планы на будущее, что бы сделал иначе.]**
